...
As regras operacionais aplicáveis ao ciclo deverão ser determinadas no início de sua execução. Alterações posteriores nas configurações globais ou específicas do professor não deverão modificar retroativamente ciclos já iniciados.

Ciclos encerrados deverão ser tratados como registros históricos e não poderão sofrer alterações automáticas decorrentes de mudanças posteriores em preços ou configurações.
...
O sistema deverá permitir controlar créditos, utilizações e limites de reposição vinculados às matrículas e aos ciclos.
...
### RF-016 — Configurações globais

O sistema deverá permitir definir configurações globais para regras operacionais do sistema.

### RF-017 — Configurações por professor

O sistema deverá permitir que professores possuam configurações específicas para determinadas regras operacionais.

Quando uma configuração específica do professor não estiver definida, o sistema deverá utilizar a configuração global correspondente.

As configurações aplicáveis a um novo ciclo deverão ser determinadas no início de sua execução.
...
Ciclos encerrados deverão preservar as informações e regras aplicadas durante sua execução e não poderão ser modificados automaticamente por alterações posteriores de preços ou configurações.
